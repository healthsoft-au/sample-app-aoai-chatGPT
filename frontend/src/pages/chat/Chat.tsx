...
const Chat = () => {
    ...
    const [showAuthMessage, setShowAuthMessage] = useState<boolean>(false); // NISM, Changed to false from true so we don't get the flash on load
            ...
            {showAuthMessage ? (
                <Stack className={styles.chatEmptyState}>
                    ...
                    <h2 className={styles.chatEmptyStateSubtitle}>
                        ...
                        and following
                        <a href="https://learn.microsoft.com/en-us/azure/app-service/scenario-secure-app-authentication-app-service#3-configure-authentication-and-authorization" target="_blank" rel="noopener noreferrer"> these instructions</a>.
                    </h2>
                                ...
                                {showLoadingMessage && (
                                    <>
                                        <div className={styles.chatMessageGpt}>
                                            <Answer
                                                answer={{
                                                    answer: "Scanning the Microsoft licensing documents...",
                                                    citations: []
                                                }}
                        ...
                        <Stack horizontal className={styles.chatInput}>
                            ...
                            <Stack>
                                {appStateContext?.state.isCosmosDBAvailable?.status !== CosmosDBStatus.NotConfigured && <CommandBarButton
                                    ...
                                    className={`commandBarButton newChatIcon ${disabledButton() ? 'commandBarButtonDisabled' : ''}`}
                                    iconProps={{ iconName: 'Add' }}
                                    onClick={newChat}
                                    disabled={disabledButton()}
                                    aria-label="start a new chat button"
                                />}
                                {/* NISM, Removed clear and new chat buttons. <CommandBarButton
                                    role="button"
                                    // styles={{
                                    //     icon: {
                                    //         color: '#FFFFFF',
                                    ...
                                    className={`commandBarButton ${appStateContext?.state.isCosmosDBAvailable?.status !== CosmosDBStatus.NotConfigured ? 'clearChatBroom' : 'clearChatBroomNoCosmos'} ${disabledButton() ? 'commandBarButtonDisabled' : ''}`}
                            ...
                            <QuestionInput
                                clearOnSend
                                placeholder="Ask a Microsoft licensing questions..."
                                disabled={isLoading}
                                onSend={(question, id) => {
